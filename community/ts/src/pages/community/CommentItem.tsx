import { CommentRequest } from "#types/request";
...
const CommentItem = ({ comment, postId }: Props) => {
  ...
  const deleteMutation = useAuthMutation<void, void>(
    `/posts/${postId}/replies/${_id}`,
    {
      method: "DELETE",
    }
  );

  const editMutation = useAuthMutation<void, CommentRequest>(
    `/posts/${postId}/replies/${_id}`,
    {
      method: "PATCH",
    }
  ...
  const editHandler = async (data: CommentRequest) => {
    if (isDirty) {
      try {
